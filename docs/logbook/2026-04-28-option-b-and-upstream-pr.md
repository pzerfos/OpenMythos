...
3. ~~**`router_bias` load balancing**~~ (pzerfos/OpenMythos#3) — **Resolved 2026-04-29** via PR #8 (merge `c41469e`) + PR #9 follow-up fixes (merge `0586f2f`). DeepSeek-V3 Algorithm 1 wired up with FSDP-safe `all_reduce`, pinned deadlock-invariant comments, robust tests. **Enabled mid-run at step 156,000** on 2026-04-29 (`router_bias_update_rate = 1e-3`, job 71939) — see `docs/logbook/2026-04-29-enable-router-bias-update.md`. Design + test details: `docs/logbook/2026-04-29-router-bias-load-balancing.md`.
4. ~~**FSDP1 → FSDP2 migration** (pzerfos/OpenMythos#6) — feasibility phase~~ — **Decided 2026-04-29**: migrate to FSDP2, skip `torch.compile`. Benchmark (job 72079) shows fsdp2 delivers a clean 15% speed-up + 6% memory reduction on the stochastic-depth production recipe vs fsdp1. `torch.compile` is unreliable on our model due to RoPE's complex-op graph breaks (36 recompilations per run) and does not meet the plan's pass bar. Full results and migration plan: `docs/logbook/2026-04-29-fsdp2-feasibility-results.md`. **Next concrete step:** `feat/fsdp2-migration` branch — rewrite FSDP wrapping and checkpoint save/load; queued behind the in-flight 10B run (job 71939) finishing.
